...
const handleConcluirEvento = async (id) => {
  if (!confirm("Deseja realmente concluir este evento?")) return;

  try {
    const response = await fetch(`${backendUrl}/events/concluirEvento/`, {
      method: "PUT",
      headers: {
    ...
    if (response.ok) {
      alert("Evento concluído com sucesso!");
       window.location.reload(); // atualiza a lista de eventos
    } else {
      const data = await response.json();
      alert("Erro: " + data.message);
    }
      ...
      <div className="card-body mx-1">
          ...
          <li className="list-group-item border-0 p-0 mb-1" style={{ backgroundColor: "var(--card-bg)", color: "var(--card-text-muted)" }}>
            ...
            >
              <div
                ...
                style={{
                  width: `${percent}%`,
                  background: "var(--mais-cultura-gradient-bg)",
                }}
              ></div>
            </div>
          </li>
        ...
        {/* Botões administrativos */}
            ...
            <button
                ...
                style={{ background: "var(--mais-cultura-gradient-bg)" }} onClick={() => handleConcluirEvento(evento.id)}
            >
                Concluir evento
            </button>
            ...
            <div className="d-flex gap-2">
                ...
                <button
                type="button"
                className="btn btn-outline-danger rounded-pill fw-medium flex-fill p-2" onClick={() => handleDeleteEvent(evento.id)}
                >
